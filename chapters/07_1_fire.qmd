...
&nbsp&nbsp $A_o$ - summan av arean för alla ventilationsöppningar           
&nbsp&nbsp $h_o$ - den viktade höjden på öppningarna enligt $h_{o}=A_v/w_o$ 
&nbsp&nbsp $w_o$ - den totala bredden på alla öppningar                     
&nbsp&nbsp $A_t$ - den totala omslutningsarean inklusive öppningar          
...
&nbsp&nbsp $\lambda$ - är värmeledningstalet för omgivande konstruktioner
&nbsp&nbsp $\rho$ - är densiteten för omgivande konstruktioner
&nbsp&nbsp $c_p$ - är den specifika värmen för omgivande konstruktioner
...
$$
T_{f}=\begin{cases}
  T_{max}-625\left(t^*-t^*_{max}x\right) & för & t^*_{max} \leq 0.5 \\
  T_{max}-250\left(3-t^*_{max}\right)\left(t^*-t^*_{max}x\right) & för & 0.5 < t^*_{max} < 2.0 \\
  T_{max}-250\left(t^*-t^*_{max}x\right) & för & t^*_{max} \geq 2.0 \\
  \end{cases}
$$
...
$$
t_{cool}=\begin{cases}
  \frac{\frac{T_{max}-T_0}{625}+t^*_{max}}{\Gamma} & för & t^*_{max} \leq 0.5 \\
  \frac{\frac{T_{max}-T_0}{250\left(3-t^*_{max}\right)}+t^*_{max}}{\Gamma} & för & 0.5 < t^*_{max} < 2.0 \\
  \frac{T_{max}-T_0}{250\Gamma}+t_{max} & för & t^*_{max} \geq 2.0 \\
  \end{cases}
$$
